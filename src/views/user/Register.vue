<template>
  <a-tabs :forceRender="true" defaultActiveKey="1" :tabBarStyle="{'text-align': 'center'}">
    <a-tab-pane key="1">
      <span slot="tab">
        <icon-font type="icon-dengluren" />
        {{ !taId ? '扫码注册' : '绑定手机号' }}
      </span>
      <!-- 扫码注册 -->
      <div v-show="!taId" id="login_container"></div>

      <!-- 用户信息 -->
      <a-form class="user-layout-login" v-show="taId" :form="form" @submit="handleSubmit">
        <a-form-item>
          <a-input
            size="large"
...
export default class Name extends Vue {
  ...
  @Prop({ default: '' }) private taId?: string
